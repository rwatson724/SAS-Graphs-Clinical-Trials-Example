/****************************************************************************************
Program:          Program 5-3.sas
SAS Version:      SAS 9.4M3
Developer:        Kriss Harris 
Date:             2020-03-29
Purpose:          Used to create the Cumulative incidence plot including hazard ratio and log-rank p-value using GTL  
                  for SAS® Graphics for Clinical Trials by Example book. 
Operating Sys:    Windows 7
Macros:           NONE
Input:            adam.adtteeff 
Output:           Output 5-3.png
...
proc template;
   define statgraph kmtemplate;
      ...
      begingraph;
	 *entrytitle "Cumulative Incidence" / textattrs=(size=11pt);
     *entrytitle "With Number of Patients at-Risk" / textattrs=(size=10pt);
	  layout overlay / xaxisopts=(label="Days from Randomisation" linearopts=(tickvaluesequence=(start=0 end=210 increment=30)))
                       yaxisopts=(label="Crude Cumulative Incidence" /*labelattrs=(size=9pt)*/);
	     stepplot x = time y = eval(1-survival) / group = stratum name="Survival";

         scatterplot x=time y=eval(1-censored) / markerattrs=(symbol=plus color=black) name="Censored"
            legendlabel="Censored";
	     scatterplot x=time y=eval(1-censored) / markerattrs=(symbol=plus) group=stratum;

         discretelegend "Censored" / location = inside autoalign = (bottomright);
		 ...
		 layout gridded / columns=2 rows = 3 border = true halign = right valign = bottom outerpad=(bottom=25px);
...
ods graphics / reset=all imagename="Output 5-3" height=3.33in width=5in;
